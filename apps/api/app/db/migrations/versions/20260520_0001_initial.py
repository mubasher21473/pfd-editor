...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("uploads_reset_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("date_trunc('month', now())")),
    ...
    op.create_table(
        "pdf_objects",
        sa.Column("id", sa.UUID(), primary_key=True, nullable=False),
        sa.Column("file_id", sa.UUID(), sa.ForeignKey("pdf_files.id", ondelete="CASCADE"), nullable=False),
